    ...
    def quote_commands(self, cmd, args, user, is_admin):
        ...
        try:
            ...
            if quote_num <= len(self.quote_list) and quote_num > 0:
                return f"Quote #{quote_num}: {self.get_quote(quote_num-1)}"
            else:
                return f"There is no quote #{quote_num}"
        except:
            pass
            ...
            if num_args >= 1:
                try:
                    ...
                    if quote_num <= len(self.quote_list) and quote_num > 0:
                        ...
                        return f"Quote #{quote_num} has been used {num_times} time{'' if num_times == 1 else 's'}"
                except:
                    pass

            return f"There are {len(self.quote_list)} quotes"

        # Check if asking for the quote list
        if cmd == 'list':
            return f"The quote list can be found at {self.settings['share-link']}"

        if cmd == 'add':
            if num_args < 1:
                return f"Not enough arguments provided"
            ...
            return f"Quote #{len(self.quote_list)} successfully added: {quote_str}"
        elif cmd == 'remove':
            if num_args < 1:
                return f"Not enough arguments provided"

            if not is_admin:
                return f"Only moderators can remove quotes"
            else:
                try:
                    quote_num = int(args[0])

                    if quote_num < 1 or quote_num > len(self.quote_list):
                        return f"Cannot remove quote #{quote_num}"
                    else:
                        self.remove_quote(quote_num-1)

                        return f"Successfully removed quote #{quote_num}"

                except:
                    return f"Quote to remove must be an integer"
        elif cmd == 'edit':
            if num_args < 1:
                return f"Not enough arguments provided"
            ...
            if not is_admin:
                ...
                try:
                    ...
                    if quote_num < 1 or quote_num > len(self.quote_list):
                        return f"Cannot edit quote #{quote_num}"
                    else:
                        ...
                        return f"Successfully edited quote #{quote_num}"
                except:
                    return f"Quote to edit must be an integer"
        else:
...
def parse_settings():
    ...
    if os.path.exists('settings.json'):
        ...
        spreadsheet_id = input('Enter the ID of the quote spreadsheet: ').strip()
        range_name = input('Enter the range of the sheet for quotes to enter: ').strip()
        read_link = input('Enter the viewing link for the quote sheet: ').strip()
